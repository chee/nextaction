...
type DragItemsUnion<T extends AnyParentType> =
	FlatChildTypesFor<T> extends infer C extends AnyChildType
		? DragAndDropItem<C>
		: never
...
export function getDropTargetIndex<T extends AnyParentType>({
	...
	const dropTargetElement = validElements.get(dropTargetIndex) as
		| HTMLElement
		| undefined
